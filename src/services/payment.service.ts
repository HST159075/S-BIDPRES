...
export async function initiateSSLCommerzPayment(bidId: string, userId: string) {
  ...
  let payment = await prisma.payment.findUnique({ where: { bidId } });
  if (!payment) {
    payment = await prisma.payment.create({
      data: { bidId, userId, totalAmount: amount, platformFee, sellerAmount, gateway: "sslcommerz", status: "pending" },
    });
  }
...
export async function initiateBkashPayment(bidId: string, userId: string) {
  ...
  if (!payment) {
    payment = await prisma.payment.create({
      data: { bidId, userId, totalAmount: amount, platformFee, sellerAmount, gateway: "bkash", status: "pending" },
    });
  }
  ...
  const tokenResp = await axios.post(base + "/token/grant",
  ...
  const payResp = await axios.post(base + "/create",
    { mode: "0011", payerReference: userId, callbackURL: process.env.API_URL + "/api/v1/payments/webhook/bkash", amount: amount.toString(), currency: "BDT", intent: "sale", merchantInvoiceNumber: payment.id },
...
export async function handleBkashCallback(query: Record<string, string>) {
  if (query.status !== "success")
    throw Object.assign(new Error("bKash payment failed."), { statusCode: 400 });
  await prisma.payment.update({
    where: { id: query.merchantInvoiceNumber },
    data: { status: "held", transactionId: query.paymentID, paidAt: new Date() },
  });
  return { success: true };
}
...
export async function confirmDelivery(auctionId: string, buyerId: string) {
  ...
  if (auction.winnerId !== buyerId)
    throw Object.assign(new Error("Only the winner can confirm."), { statusCode: 403 });

  const payment = await prisma.payment.findFirst({
    where: { bid: { auctionId }, status: "held" },
  });
  if (!payment)
    throw Object.assign(new Error("No held payment found."), { statusCode: 404 });

  await prisma.$transaction([
    prisma.payment.update({ where: { id: payment.id }, data: { status: "released", releasedAt: new Date() } }),
    prisma.user.update({ where: { id: buyerId }, data: { purchaseCount: { increment: 1 } } }),
  ]);
